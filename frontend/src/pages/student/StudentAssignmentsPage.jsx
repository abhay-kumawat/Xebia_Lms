...
const XEBIA_PORTAL_ASSIGNMENTS = [
  {
    ...
    totalMarks: 50,
    passingMarks: 35,
    instructions: 'Write a short walkthrough of the Xebia portal, highlighting the Home, Courses, Assignments, and Assessments experience. Include one improvement suggestion for learners and one for admins.',
    allowedFormats: ['PDF', 'DOCX'],
    rubricCriteria: ['Portal navigation clarity', 'Understanding of learner journeys', 'Quality of improvement suggestion']
  },
  {
    id: 'xebia-assign-2',
    ...
    totalMarks: 75,
    passingMarks: 50,
    instructions: 'Prepare a release checklist for a new Xebia learning path. Cover course structure, module naming, content order, assessment readiness, and expected learner outcomes.',
    allowedFormats: ['PDF', 'PPTX'],
    rubricCriteria: ['Readiness coverage', 'Content structure', 'Practicality of recommendations']
  },
  {
    id: 'xebia-assign-3',
    ...
    totalMarks: 100,
    passingMarks: 70,
    instructions: 'Submit a capstone plan for a portal-based learning journey. Include the project objective, milestones, deliverables, and how you would track progress inside the Xebia portal.',
    allowedFormats: ['PDF', 'ZIP', 'DOCX'],
    rubricCriteria: ['Milestone planning', 'Portal usage strategy', 'Clarity of deliverables']
  }
];
...
export default function StudentAssignmentsPage() {
  ...
  const assignmentsToShow = liveAssignments.length > 0
    ? [...XEBIA_PORTAL_ASSIGNMENTS, ...liveAssignments]
    : XEBIA_PORTAL_ASSIGNMENTS;
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex flex-col items-end gap-1">
                    {assign.totalMarks ? (
                      <span className="rounded-full bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300 px-3 py-1 text-[10px] font-extrabold uppercase">
                        {assign.totalMarks} pts
                      </span>
                    ) : null}
                ...
                <div className="mt-3 flex flex-wrap gap-2 text-[10px] font-bold uppercase tracking-wide text-slate-500 dark:text-slate-300">
                  <span className="rounded-full bg-slate-100 dark:bg-[#111827] px-2.5 py-1">Pass {assign.passingMarks} pts</span>
                  {assign.allowedFormats?.slice(0, 3).map(format => (
                    <span key={format} className="rounded-full bg-slate-100 dark:bg-[#111827] px-2.5 py-1">{format}</span>
                  ))}
                </div>
