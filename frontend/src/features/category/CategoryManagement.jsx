  ...
  return (
    <div className="relative">
      <select
        ...
        className="appearance-none cursor-pointer rounded-xl border bg-white/[0.04] border-white/[0.08] py-2 pl-3 pr-8 text-sm text-white focus:outline-none hover:border-white/[0.14] transition-all"
      >
        {options.map((o) => (
    ...
    <div className="flex min-h-screen flex-col bg-transparent text-slate-100 transition-colors">
      ...
      <div className="flex items-center justify-between px-8 py-5 bg-[#0C0D12]/40 border-b border-white/[0.06] backdrop-blur-xl">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-505">Manage all learning categories on the platform</p>
        </div>
        <button
          type="button"
          onClick={() => navigate('/admin/categories/new')}
          className="flex items-center gap-2 rounded-xl px-5 py-2.5 text-xs font-bold text-white bg-gradient-to-tr from-purple-600 to-indigo-600 shadow-md hover:opacity-90 hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer border-0"
        >
      ...
      <div className="flex-1 px-8 py-6">
        ...
        <div className="mb-5 flex items-center gap-3">
          {/* Search */}
          <div
            className="flex flex-1 items-center gap-2 rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 py-2 hover:border-white/[0.14] transition-all"
            style={{ maxWidth: 280 }}
          >
          ...
          {/* View toggle */}
          <div
            className="ml-auto flex items-center gap-1 rounded-xl border border-white/[0.08] bg-white/[0.04] p-1"
          >
            <button
              type="button"
              onClick={() => setView('grid')}
              className="flex h-7 w-8 items-center justify-center rounded-lg transition-colors cursor-pointer"
              style={view === 'grid' ? { backgroundColor: 'rgba(168,85,247,0.2)' } : {}}
              title="Grid view"
            >
              <LayoutGrid className="h-3.5 w-3.5" style={{ color: view === 'grid' ? '#10b981' : '#64748b' }} />
            </button>
            <button
              type="button"
              onClick={() => setView('table')}
              className="flex h-7 w-8 items-center justify-center rounded-lg transition-colors cursor-pointer"
              style={view === 'table' ? { backgroundColor: 'rgba(168,85,247,0.2)' } : {}}
              title="List view"
            >
              <List className="h-3.5 w-3.5" style={{ color: view === 'table' ? '#10b981' : '#64748b' }} />
            </button>
          </div>
