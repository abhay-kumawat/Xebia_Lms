...
export default function CategoryCard({ category, courseCount, onEdit, onDelete, onView }) {
  return (
    <motion.div
      ...
      style={category.color ? { borderTop: `3px solid ${category.color}` } : undefined}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 mb-2">
            <CourseStatusBadge status={category.isActive ? 'active' : 'inactive'} />
            {category.deletedAt && <CourseStatusBadge status="archived" />}
          </div>
          <h3 className="font-semibold text-brand-text-primary dark:text-slate-100 truncate">
  ...
  return (
    <tr className="border-b border-brand-border dark:border-slate-800 hover:bg-brand-surface/50 dark:hover:bg-slate-850/40 transition-colors">
      <td className="px-4 py-3">
        <button type="button" onClick={() => onView(category)} className="font-medium text-brand-primary dark:text-brand-secondary hover:underline text-left flex items-center gap-2">
          {category.color && (
      ...
      <td className="px-4 py-3"><CourseStatusBadge status={category.isActive ? 'active' : 'inactive'} /></td>
        ...
        <div className="relative" ref={menuRef}>
          ...
          {menuOpen && (
            <div className="absolute right-0 top-full z-10 mt-1 w-36 rounded-xl border border-brand-border dark:border-slate-800 bg-white dark:bg-slate-900 py-1 shadow-card">
              ...
              <button type="button" className="w-full px-3 py-2 text-left text-sm text-red-650 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/20" onClick={() => { onDelete(category); setMenuOpen(false); }}>Delete</button>
            </div>
          )}
        </div>
      </td>
